        ...
        <div className="relative min-h-[1200px]">
          ...
          <div className="space-y-12">
            {milestones?.map((milestone, index) => (
              <div key={milestone?.year} className={`flex items-center min-h-[200px] flex-col sm:${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}>
                <div className={`w-full sm:w-[45%] ${index % 2 === 0 ? 'sm:pr-4 lg:pr-8' : 'sm:pl-4 lg:pl-8'}`}>
                  <div className="bg-card rounded-xl p-4 lg:p-6 shadow-soft hover:shadow-lift transition-all duration-300 border border-border contain-layout">
                    <div className="flex items-center space-x-3 mb-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xl lg:text-2xl font-bold text-foreground">{milestone?.year}</div>
                        <div className="text-xs lg:text-sm text-muted-foreground">Milestone</div>
                      </div>
                    </div>
                    <h3 className="text-lg lg:text-xl font-semibold text-foreground mb-2">{milestone?.title}</h3>
                    <p className="text-sm text-muted-foreground">{milestone?.description}</p>
                  </div>
                </div>
                
                <div className="relative z-10 flex items-center justify-center w-8 h-8 sm:w-8 lg:w-16 flex-shrink-0 my-4 sm:my-0">
